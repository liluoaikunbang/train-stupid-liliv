...
<script>
    ...
    import { addTrainLogs } from '../TrainLogUtils'
    export default {
        ...
        setup(){
            ...
            const lili_name = current_lili.name
            ...
            // 初始化数据与状态
            if(store.state.current_id != null&&current_lili.whereabouts.state != '派遣中'){
                handleFlags()
            }
            // 每个按钮执行后的通用操作
            const handleButtons=()=>{
            ...
            // 捆绑按键的通用操作
            const handleBondage=()=>{
                current_lili = changeAttributes({input_lili:current_lili, add_state:['捆绑']})
                handleButtons()
            }
            // 解缚按键的通用操作
            const handleUntie=()=>{
                // 判断姿势
                if(!current_lili.soon.bondage.arm){current_lili.soon.pose.arm = '自由'}
                if(!current_lili.soon.bondage.finger){current_lili.soon.pose.finger = '自由'}
                if(!current_lili.soon.bondage.waist){current_lili.soon.pose.waist = '自由'}
                if(!current_lili.soon.bondage.lap&&!current_lili.soon.bondage.shins){current_lili.soon.pose.leg = '自由'}
                if(!current_lili.soon.bondage.foot&&!current_lili.soon.bondage.toe){current_lili.soon.pose.foot = '自由'}
                // 判断捆绑状态
                if(!current_lili.soon.bondage.eye&&!current_lili.soon.bondage.ear&&
                    !current_lili.soon.bondage.nose&&!current_lili.soon.bondage.mouth&&
                    !current_lili.soon.bondage.neck&&!current_lili.soon.bondage.arm&&
                    !current_lili.soon.bondage.finger&&!current_lili.soon.bondage.waist&&
                    !current_lili.soon.bondage.lap&&!current_lili.soon.bondage.shins&&
                    !current_lili.soon.bondage.foot&&!current_lili.soon.bondage.toe){
            ...
            const clickBlindfold=()=>{ // 戴上眼罩
                store.commit('addTime', 1)
                addTrainLogs('blindfold')
            ...
            const untieBlindfold=()=>{ // 解开眼罩
                addLog(false, `${name}给${current_lili.name}摘下了眼罩。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_eye:-10})
                flags.blindfold_flag = 1
            ...
            const clickBallgag=()=>{ // 戴上口球
                store.commit('addTime', 3)
                addLog(true, train_logs.bondage.ballgag, true)
            ...
            const untieBallgag=()=>{ // 摘下口球
                addLog(false, `${name}给${current_lili.name}摘下了口球。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_mouth:-5})
                flags.ballgag_flag = 1
            ...
            const clickEarplug=()=>{ // 戴上耳塞
                store.commit('addTime', 1)
                addLog(true, train_logs.bondage.earplug, true)
            ...
            const untieEarplug=()=>{ // 摘下耳塞
                addLog(false, `${name}给${current_lili.name}摘掉了耳塞。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_ear:-5})
                flags.earplug_flag = 1
            ...
            const clickFrontHandBinding=()=>{ // 捆绑前手缚
                store.commit('addTime', 10)
                addLog(true, train_logs.bondage.front_hand_binding, true)
            ...
            const untieFrontHandBinding=()=>{ // 解开前手缚
                addLog(false, `${name}给${current_lili.name}解开了双手的束缚。`, true)
            ...
            const clickBackHandBinding=()=>{ // 捆绑后手缚
                store.commit('addTime', 10)
                addLog(true, train_logs.bondage.back_hand_binding, true)
            ...
            const clickBoxBinding=()=>{ // 日式盒缚
                store.commit('addTime', 20)
                addLog('bondage', `${name}将${current_lili.name}的双手在身后平行反绑，多余的绳索缠绕过胸部上下，将她的胸胸勒得更加丰满。`, true)
            ...
            const clickStraightArmBinding=()=>{ // 欧式直臂
                store.commit('addTime', 20)
                addLog('bondage', `${name}将${current_lili.name}的双手反剪，从手腕到大臂均并拢捆紧，让她的双臂无法分开。`, true)
            ...
            const clickBackPrayerBinding=()=>{ // 后手观音
                ...
                addLog('bondage', `${name}将${current_lili.name}的双手在背后吊高，手指向上呈反拜姿势，将手腕到手肘分别捆紧，大小臂也折叠紧缚在一起。`, true)
            ...
            const clickTapeWrapFingers=()=>{ // 胶带包手
                store.commit('addTime', 10)
                addLog('bondage', `${name}将${current_lili.name}的双手握拳，用胶带包裹成两个圆球。`, true)
            ...
            const untieTapeWrapFingers=()=>{ // 解开胶带包手
                addLog(false, `${name}给${current_lili.name}解开了手指的束缚。`, true)
            ...
            const clickLegBinding=()=>{ // 并腿缚
                store.commit('addTime', 10)
                addLog('bondage', `${name}将${current_lili.name}的双腿并拢捆绑起来。`, true)
            ...
            const untieLegBinding=()=>{ // 解开并腿缚
                addLog(false, `${name}给${current_lili.name}解开了双腿的束缚。`, true)
            ...
            const clickOpenLegBinding=()=>{ // M字开腿缚
                store.commit('addTime', 10)
                addLog('bondage', `${name}将${current_lili.name}的双腿分别折叠捆紧。`, true)
            ...
            const clickBallBinding=()=>{ // 桃缚
                store.commit('addTime', 20)
                addLog('bondage', `${name}将${current_lili.name}的双腿分别折叠捆紧后将脚踝交叉捆绑，并连接绳索到脖颈，将${current_lili.name}头部和脚踝逐渐拉近。`, true)
            ...
            const untieBallBinding=()=>{ // 解开桃缚
                addLog(false, `${name}给${current_lili.name}解开了桃缚。`, true)
            ...
            const clickK9Binding=()=>{ // K9拘束衣
                store.commit('addTime', 20)
                addLog('bondage', `${name}给${current_lili.name}套上K9拘束服，打扮成四肢着地的宠物样子。`, true)
            ...
            const untieK9Binding=()=>{ // 解开K9拘束衣
                addLog(false, `${name}给${current_lili.name}解开了K9拘束衣。`, true)
            ...
            const clickHorseBinding=()=>{ // 马奴拘束
                store.commit('addTime', 20)
                addLog('bondage', `${name}给${current_lili.name}打扮成马驹的样子。`, true)
            ...
            const untieHorseBinding=()=>{ // 解开马奴拘束
                addLog(false, `${name}给${current_lili.name}解开了马奴拘束衣。`, true)
            ...
            const clickXBinding=()=>{ // X架拘束
                store.commit('addTime', 20)
                addLog('bondage', `${name}将${current_lili.name}禁锢到X型拘束架上。`, true)
            ...
            const untieXBinding=()=>{ // 解开X架拘束
                addLog(false, `${name}将${current_lili.name}从X型拘束架上放下来。`, true)
            ...
            const clickMermaidBinding=()=>{ // 乳胶人鱼拘束衣
                store.commit('addTime', 30)
                addLog('bondage', `${name}给${current_lili.name}套上乳胶人鱼拘束衣。`, true)
            ...
            const untieMermaidBinding=()=>{ // 解开乳胶人鱼拘束衣
                addLog(false, `${name}给${current_lili.name}脱掉乳胶人鱼拘束衣。`, true)
            ...
            const clickMummifiedHeadBinding=()=>{ // 绷带木乃伊（包头）
                store.commit('addTime', 30)
                addLog('bondage', `${name}用绷带将${current_lili.name}从头到脚层层包裹起来。`, true)
            ...
            const untieMummifiedHeadBinding=()=>{ // 解开绷带木乃伊（包头）
                addLog('bondage', `${name}给${current_lili.name}解开了木乃伊拘束。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_ear:-2, bondage_mouth:-1, bondage_nose:-3, bondage_eye:-10,
                    bondage_arm:-10, bondage_finger:-10, bondage_lap:-10, bondage_shins:-10, bondage_foot:-10, bondage_toe:-10})
            ...
            const clickLigatureMummifiedBinding=()=>{ // 绷带木乃伊（不包头）
                store.commit('addTime', 30)
                addLog('bondage', `${name}用绷带将${current_lili.name}脖颈之下层层包裹起来。`, true)
            ...
            const untieLigatureMummifiedBinding=()=>{ // 解开绷带木乃伊（不包头）
                addLog(false, `${name}给${current_lili.name}解开了木乃伊拘束。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_arm:-10, bondage_finger:-10, bondage_lap:-10, 
                    bondage_shins:-10, bondage_foot:-10, bondage_toe:-10})
            ...
            const clickTapeMimmifiedBinding=()=>{ // 胶带木乃伊（不包头）
                store.commit('addTime', 30)
                addLog('bondage', `${name}用胶带将${current_lili.name}脖颈之下层层包裹起来。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_arm:10, bondage_finger:10, bondage_lap:10, 
                    bondage_shins:10, bondage_foot:10, bondage_toe:10})
                handleMummifiedBinding()
            ...
            const clickOneStockings=()=>{ // 一层丝袜拘束
                store.commit('addTime', 5)
                addLog('bondage', `${name}用一层丝袜将${current_lili.name}包裹起来。`, true)
            ...
            const untieOneStockings=()=>{ // 解开一层丝袜拘束
                addLog(false, `${name}给${current_lili.name}解开了丝袜拘束。`, true)
                current_lili = changeAttributes({input_lili:current_lili, bondage_nose:-1, bondage_arm:-1, bondage_finger:-1, bondage_lap:-1, 
                    bondage_shins:-1, bondage_foot:-1, bondage_toe:-1})
            ...
            const clickTwoStockings=()=>{ // 两层丝袜拘束
                store.commit('addTime', 10)
                addLog('bondage', `${name}用两层丝袜将${current_lili.name}包裹起来。`, true)
            ...
            const clickThreeStockings=()=>{ // 三层丝袜拘束
                store.commit('addTime', 15)
                addLog('bondage', `${name}用三层丝袜将${current_lili.name}包裹起来。`, true)
            ...
            const clickFiveStockings=()=>{ // 五层丝袜拘束
                store.commit('addTime', 25)
                addLog('bondage', `${name}用五层丝袜将${current_lili.name}包裹起来。`, true)
            ...
            const clickUntieAll=()=>{
                store.commit('addTime', 10)
                addLog(false, `${name}给${current_lili.name}解开了全身所有的束缚。`, false)
