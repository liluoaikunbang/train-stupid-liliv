    ...
    export default {
        ...
        setup(){
            ...
            const clickExposure=()=>{ // 拘束露出
                ...
                addLog('modify', `${name}将${current_lili.name}捆绑好后强行牵到了外面，让${current_lili.name}带着满身拘束暴露在人群中。`, true)
            ...
            const clickDisguise=()=>{ // 伪装拘束
                store.commit('addTime', 60)
                addLog('modify', `${name}将${current_lili.name}捆绑好后强行牵到了外面，但在她身上披了一件衣服，挡住了全身的拘束。`, true)
            ...
            const clickWalk=()=>{ // 步行训练
                store.commit('addTime', 60)
                addLog('modify', `${name}将${current_lili.name}捆绑好后牵到了院子里快步走了几圈。`, true)
                current_lili = changeAttributes({input_lili:current_lili, sen_foot:-1, manners_lady:2})
                handleButtons()
            }

            const clickSwim=()=>{ // 游泳训练
                store.commit('addTime', 40)
                addLog('modify', `${name}将${current_lili.name}捆绑好后牵到了后院游泳池中。`, true)
            ...
            const clickPlaceCommon=()=>{ // 普通拘束放置
                store.commit('addTime', 60)
                addLog('modify', `${name}将${current_lili.name}捆绑完全后放置在院子中。`, true)
                current_lili = changeAttributes({input_lili:current_lili, servility:10, favorability:-5})
                handleButtons()
            }
            ...
            const clickPlaceSensory=()=>{ // 感官封闭放置
                store.commit('addTime', 60)
                addLog('modify', `${name}将${current_lili.name}捆绑完全感官封闭后放置在院子中。`, true)
