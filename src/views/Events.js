...
import { deleteArchive } from '../components/Layout/Archive'
import { judgeFlag, changeAttributes } from '../components/Attributes/Attributes'
import { judgeSpecialty, everydayDispatch } from '../components/LiLiSold/SaleUtils'
import { addLog } from '../components/LogArea/LogUtils'

// 天数判定事件
export function handleNextDayEvents(){
    let current_lili = store.state.current_lili
    // 当天结束时的事件
    everydayDispatch() // 劳务派遣的工资判定，用于每日增加球币
    if(current_lili.state.includes('胀奶')){ // 胀奶
        addLog('event', `${current_lili.name}的胀奶状态没有及时排除，她感觉胸部胀胀的很难受，健康程度下降了。`, true)
        current_lili = changeAttributes({input_lili:current_lili, healthy:-20})
    }

    // 第二天开始时的事件
    if(!current_lili.state.includes('胀奶')){ // 恢复
        addLog('event', `经过了一夜的休息，${current_lili.name}感觉精神焕发，健康程度上升了。`, true)
        current_lili = changeAttributes({input_lili:current_lili, healthy:-20})
    }
    if(current_lili.state.includes('泌乳')){ // 开始胀奶
        addLog('event', `在空孕催乳剂的作用下，${current_lili.name}开始胀奶了，增加胀奶状态。`, true)
...
export function handleNextHourEvents(){
    ...
    if(judgeFlag({current_lili, pleasure_breast:[1, Infinity]})){
        addLog('event', `胸胸受到刺激，${current_lili.name}的性欲增加了。`, true)
    ...
    if(judgeFlag({current_lili, pleasure_vagina:[1, Infinity]})){
        addLog('event', `下体受到刺激，${current_lili.name}的性欲增加了。`, true)
    ...
    if(judgeFlag({current_lili, need_state:['发情']})){ // 发情
        addLog('event', `${current_lili.name}持续发情，但仍然没有得到主人的排解，奴性增加了，但好感度下降了。`, true)
...
export function handleInstantEvents(){
    ...
    if(judgeFlag({current_lili, lust:[100, Infinity]})){ // 高潮
        addLog('event', `${current_lili.name}未经主人允许达到了高潮，奴性下降了。`, true)
    ...
    // 根据属性变化对当前的特长进行增删
    current_lili = judgeSpecialty(current_lili)
